import { FormatOptions , toFormatedNumber} from '@edsolater/fnkit'
...
export function toUsdVolume(
  ...
  const formatFn = (n: Fraction) =>
    toFormatedNumber(n.toFixed(options?.decimalPlace ?? 2, options?.format, options?.rounding), {
      fractionLength: 'auto',
      ...options
    })
